...
int main(int argc, char** argv)
{
    ...
    if (tankNet::isConnected() && tankNet::isProvisioned())
    {
        ...
        while (sfw::stepContext() && tankNet::isConnected() && tankNet::isProvisioned())
        {
			...
			/*for (int i = 0; i < state->tacticoolCount; ++i)
			{
				if (state->tacticoolData[i].lastKnownPosition[0] < state->position[0])
				{
					ex.tankMove = tankNet::TankMovementOptions::LEFT;
				}
				if (state->tacticoolData[i].lastKnownPosition[0] > state->position[0])
				{
					ex.tankMove = tankNet::TankMovementOptions::RIGHT;
				}
			}*/
...
for (int i = 0; i < state->tacticoolCount; ++i)
		{
			state->tacticoolData[i].playerID;
		}
*/